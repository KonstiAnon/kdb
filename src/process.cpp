...
namespace {
    void exit_with_perror(kdb::pipe &channel, std::string const &prefix) {
...
std::unique_ptr<kdb::process> kdb::process::attach(pid_t pid) {
    if (pid == 0) {
...
kdb::stop_reason::stop_reason(int wait_status) {
...
kdb::stop_reason kdb::process::wait_on_signal() {
    int wait_status;
    int options = 0;
